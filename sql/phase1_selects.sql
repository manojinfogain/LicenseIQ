/*
================================================================================
PHASE 1: READ-ONLY STORED PROCEDURES (12 SPs)
================================================================================
...
BEGIN
    ...
    FROM dbo.Platform p
    LEFT JOIN dbo.PlatformContract pc ON p.id = pc.platform_id
    ORDER BY p.name;
    
    -- Return seat snapshots
    SELECT id, platform_id, snapshot_date, seats
    FROM dbo.PlatformSeatSnapshot
...
BEGIN
    ...
    LEFT JOIN dbo.ApprovalHistory ah ON lr.id = ah.request_id
...
BEGIN
    ...
    WHERE (employee_id = @EmployeeId OR 
           employee_id IN (SELECT id FROM dbo.Employee WHERE employee_code = @StaffId))
...
BEGIN
    ...
    SELECT ewm.id, ewm.emp_staffid, ewm.user_id, ewm.role_id, ewm.is_active,
           ewm.scope_ref_id, r.code AS role_code, r.name AS role_name
    FROM dbo.EmployeeWiseRoleMapping ewm
    LEFT JOIN dbo.Role r ON ewm.role_id = r.id
...
-- ============================================================================
-- Indexes for Phase 1 SPs
-- ============================================================================

-- Index for open alerts lookup (SP-3, SP-12)
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_Alert_EmployeeStatus')
BEGIN
    CREATE INDEX idx_Alert_EmployeeStatus ON dbo.Alert (employee_id, status, created_at DESC);
END;

-- Index for pending requests lookup (SP-5, SP-10)
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_LicenseRequest_Status')
BEGIN
    CREATE INDEX idx_LicenseRequest_Status ON dbo.LicenseRequest (employee_id, approval_status, created_at DESC);
END;

-- Index for active allocations (SP-9)
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_LicenseAllocation_Status')
BEGIN
    CREATE INDEX idx_LicenseAllocation_Status ON dbo.LicenseAllocation (employee_id, platform_id, status, revoked_date);
END;

-- Index for pending queue lookup (SP-8, SP-11)
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_QueueItem_Status')
BEGIN
    CREATE INDEX idx_QueueItem_Status ON dbo.QueueItem (employee_id, platform_id, action_type, status);
END;

-- Index for role mapping lookup (SP-13)
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_EmployeeWiseRoleMapping_StaffId')
BEGIN
    CREATE INDEX idx_EmployeeWiseRoleMapping_StaffId ON dbo.EmployeeWiseRoleMapping (emp_staffid, is_active);
END;

PRINT '✓ Phase 1 SPs created successfully (12 SPs + 5 indexes)';
